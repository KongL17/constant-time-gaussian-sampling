...
//gcc -Wextra -O3 -fomit-frame-pointer -march=native -o sample_no_96 sampler-no-opt-96.c
...
int main(){
	...
	for(i=0;i<repeat;i++){	
        ...
        memset(out_t,0,5*sizeof(uint64_t));  
		...
		out_t[2] = (~bit[93]&~bit[92]);

		out_t[1] = (bit[93]&~bit[92]) | (~bit[94]&~bit[92]);

		out_t[0] = (~bit[93]&~bit[92]) | (~bit[94]&~bit[92]);
